"""
CloudFront Security Middleware
Provides optional origin verification for CloudFront distributions.

NOTE: Origin Access Identity (OAI) is not available on Free tier CloudFront plans.
If you're on Free tier, this middleware is optional - the signed URLs and custom 
auth headers provide sufficient security.

For paid CloudFront plans with OAI enabled, enable CLOUDFRONT_ORIGIN_SECRET to 
